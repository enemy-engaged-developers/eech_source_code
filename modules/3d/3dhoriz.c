...
void destroy_all_3d_horizon_images ( env_3d *env )
{
	...
	if ( env->horizon_point_colours )
	{
		safe_free ( env->horizon_point_colours );
	}
}
			...
			if ( !outcode_3d_horizon_polygon ( this_face->number_of_points, point_list, object_base->points_base, &outcode ) )
			{
				...
				if ( outcode == 0 )
				{
					...
					for ( count = 0; count < this_face->number_of_points; count++ )
					{
						...
						destination_vertices[count].specular = 0;	//D3DCOLOR_RGBA ( 0, 0, 0, 0 );	//vert->colour;
					}
						...
						if ( poly )
						{
							...
							if ( poly )
							{
								...
								for ( count = 0; count < number_of_points; count++ )
								{
									...
									destination_vertices[count].specular = D3DCOLOR_RGBA ( 0, 0, 0, 0 );
			...
			if ( !outcode_3d_horizon_polygon ( this_face->number_of_points, point_list, object_base->points_base, &outcode ) )
			{
				...
				if ( outcode == 0 )
				{
					...
					for ( count = 0; count < this_face->number_of_points; count++ )
					{
						...
						destination_vertices[count].specular = 0;	//D3DCOLOR_RGBA ( 255, 0, 0, 0 );	//vert->colour;
					}
