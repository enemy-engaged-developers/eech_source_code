...
enum SUB_OBJECT_SEARCH_RESULT_TYPES find_object_3d_sub_object ( struct OBJECT_3D_SUB_OBJECT_SEARCH_DATA *search )
{
	...
	if ( !search->loaded )
	{
		if ( !ogre_scene_find ( &search->search_object->vp, search->sub_object_index, &search->saved_search ) )
		{
			return SUB_OBJECT_SEARCH_RESULT_OBJECT_NOT_FOUND;
		}
		search->loaded = TRUE;
	}
	if ( ( unsigned ) search->search_depth >= search->saved_search.number_of_subobjects )
	{
		return SUB_OBJECT_SEARCH_RESULT_OBJECT_NOT_FOUND;
	}
...
enum SUB_OBJECT_SEARCH_RESULT_TYPES find_object_3d_sub_object_from_sub_object ( struct OBJECT_3D_SUB_OBJECT_SEARCH_DATA *parent_search, struct OBJECT_3D_SUB_OBJECT_SEARCH_DATA *search )
{
	...
	if ( !search->loaded )
	{
		if ( !ogre_scene_find2 ( &parent_search->search_object->vp, search->sub_object_index, parent_search->result_sub_object - parent_search->search_object->vp.elements, &search->saved_search ) )
		{
			return SUB_OBJECT_SEARCH_RESULT_OBJECT_NOT_FOUND;
		}
		search->loaded = TRUE;
