...
LPTLVERTEX get_d3d_vertices_address ( vertex *polygon, int *return_number_of_vertices )
{
...
LPTLVERTEX get_d3d_vertices_points_address ( int number_of_vertices )
{
...
LPTLVERTEX get_d3d_new_vertices_points_address ( int number_of_vertices, int *offset )
{
...
LPTLVERTEX get_d3d_line_vertices_points_address ( void )
{
...
LPTLVERTEX get_d3d_point_vertices_points_address ( int number_of_points )
{
	LPTLVERTEX
		vertices;
	...
	f3d_vertex_lock ( d3d.point_vertex_buffer, D3DLOCK_NOSYSLOCK, ( LPVOID * ) &vertices );
...
LPTLVERTEX get_d3d_alpha_vertex_buffer_vertices ( int buffer )
{
	LPTLVERTEX
		vertices;
	...
	f3d_vertex_lock ( d3d.alpha_vertex_buffer[buffer], D3DLOCK_NOSYSLOCK, ( LPVOID * ) &vertices );
...
/*			if (	( d3d.triangle_buffers[count].texture.texture == data.texture ) &&
	...
	if ( !d3d.triangle_buffer->vertices )
	{
		...
		f3d_vertex_lock ( d3d.triangle_buffer->buffer, D3DLOCK_NOSYSLOCK /*| D3DLOCK_DISCARD*/, ( LPVOID * ) &d3d.triangle_buffer->vertices );
		f3d_index_lock ( d3d.triangle_buffer->ibuffer, D3DLOCK_NOSYSLOCK /*| D3DLOCK_DISCARD*/, &d3d.triangle_buffer->indices );
	}

	ASSERT ( d3d.triangle_buffer->vertices );
	ASSERT ( d3d.triangle_buffer->indices );
}
...
void get_d3d_line_vertex_buffer ( void )
{
	...
	if ( !d3d.line_buffer->vertices )
	{
		...
		f3d_vertex_lock ( d3d.line_buffer->buffer, D3DLOCK_NOSYSLOCK /*| D3DLOCK_DISCARD*/, ( LPVOID * ) &d3d.line_buffer->vertices );
		f3d_index_lock ( d3d.line_buffer->ibuffer, D3DLOCK_NOSYSLOCK /*| D3DLOCK_DISCARD*/, &d3d.line_buffer->indices );
	}

	ASSERT ( d3d.line_buffer->vertices );
	ASSERT ( d3d.line_buffer->indices );
}
