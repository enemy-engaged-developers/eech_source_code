...
#define MAX_ACTIVATE_MESSAGE_FUNCTIONS 256 //es
...
int
	...
	number_pre_activate_message_functions = 0,//es
	number_post_activate_message_functions = 0,//es
...
static long call_pre_activate_function_routine ( void *data ); //es

static long call_post_activate_function_routine ( void *data ); //es
...
static long application_window_proc ( SDL_Event *data );
...
long application_window_proc ( SDL_Event *data )
{
		...
		for ( count = 0; count < number_system_message_functions; count++ )
		{
			...
			if ( system_message_functions[count].parameter == message )
			{
				...
				if ( !bExiting )
				{
					...
					return ( system_message_functions[count].function ( data ) );
				}
				else
				{
...
*/
	...
	application_window = (SDL_Surface *) SDL_SetVideoMode ( 640, 480, 32, SDL_OPENGL );
...
int start_application ( void *data )
//void start_application ( void ) //es, win32 does this
{
...
//	application_main ( argc, argv );// defined in aphavoc/source/project.c
}
