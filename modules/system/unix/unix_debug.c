...
//static HWND
static int //FIXME: Must determine correct datatype
...
void initialise_debug_system ( int logging )
{
	...
	debug_initialise_message_id = 0;//FIXME: get_unique_message_id ();
...
static void deinitialise_debug_system ( void )
{
	...
	if ( debug_monitor_auto_update )
	{
		...
		//OLDCODE: SetThreadPriority ( application_monitor_thread, THREAD_PRIORITY_IDLE );
...
//The return code for the thread function is placed in the area pointed to by status, if status is not NULL.
		SDL_WaitThread( application_monitor_thread , NULL /*int *status*/ );
	}
...
//FIXME:		update_window ( application_debug_log_window );

//FIXME:		update_window ( application_debug_watch_window );

//FIXME:		update_window ( application_debug_filtered_log_window );
...
//FIXME:	if (file_exist (new_file_name))
	{
...
//FIXME: 	if (!file_exist (debug_log_file_name))
	{
