...
struct PLAYER_WARZONE_LOG
{
	struct PLAYER_WARZONE_LOG* next;
	char* warzone_name;
	unsigned missions;
	float flying_seconds;
};

typedef struct PLAYER_WARZONE_LOG player_warzone_log;
...
struct PLAYER_SIDE_LOG_TYPE
{
	...
	// new values, have to be last in struct to not screw up reading of old log format!
	float
		gunship_flying_seconds[NUM_GUNSHIP_TYPES];

	unsigned short
		gunship_missions[NUM_GUNSHIP_TYPES];

	unsigned int
		helicopters_lost;

	player_warzone_log* warzone_log;
};
...
extern void inc_player_log_deaths (int side, player_log_type *log);
extern void inc_player_log_helicopters_lost(int side, player_log_type *log);

extern void backup_current_player_log(void);
extern int restore_log_from_backup(void);  // returns TRUE if log was restored
