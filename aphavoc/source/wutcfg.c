...
char session_WUT_filename [128]	= "\0"; // Jabberwock 031115 WUT server side
	 ...
	 while (buf1[0] != '\0' && buf1[0] != '\n' && !(strcmp(buf1,"Ground Vehicles, Ships, Infantry")==0))
	 {
		  ...
		  for (i = 0; i < NUM_ENTITY_SUB_TYPE_AIRCRAFT; i++)
		  {
				...
				if (strcmp(aircraft_database[i].short_display_name, ent) == 0)
				{
					 ...
					 //see: \aphavoc\source\entity\mobile\aircraft\ac_dbase.h
...
//553 = "WEAPON_CONFIG_TYPE_CIS_INFANTRY_SAM_KNEELING";
	 ...
	 while (buf1[0] != '\0' && buf1[0] != '\n' && !(strcmp(buf1,"Keysites")==0))
	 {
		  ...
		  for (i = 0; i < NUM_ENTITY_SUB_TYPE_WEAPONS; i++)
		  {
			  if (strcmp(weapon_database[i].full_name, ent) == 0)
			  {
				...
				if ( strstr(buf1, "Damage Soft"                         )) { weapon_database[i].damage_capability = d2; }                
				if ( strstr(buf1, "Damage Hard"                         )) { weapon_database[i].armor_penetration_capability = d2; }                
				...
				if ( strstr(buf1, "Flight Profile or self destruction"  )) { weapon_database[i].flight_profile_or_self_destr = d2; }               
				if ( strstr(buf1, "Weapon Class"                        )) { weapon_database[i].weapon_class = d2; }                          
				if ( strstr(buf1, "Start Weight {kg}"                   )) { weapon_database[i].start_weight = v2; }                                     
				if ( strstr(buf1, "End Weight {kg}"                     )) { weapon_database[i].final_weight = v2; }                                     
				...
				if ( strstr(buf1, "Burn Time {s}"                       )) { weapon_database[i].boost_time = v2; }                                  
	 ...
	 while (buf1[0] != '\0' && buf1[0] != '\n' && !(strcmp(buf1,"Groups")==0))
	 {
		  ...
		  // search through all the keysite list
		  // see aphavoc\source\entity\system\en_types\en_sbtyp.h
		  ...
		  {        	
				...
				if (strcmp(keysite_database[i].full_name, ent) == 0)
				{
					...
					char *q = buf1+83;
					 ...
					 // see keysite properties: \aphavoc\source\entity\special\keysite\ks_dbase.h         
...
// this is not correct                 
					 ...
					 { 
						//VJ 040122 fixed a few bugs here: starting bit is 6 not 4
						 //DO NOT USE keysite_database[i].assign_task_count = (0 | (d2 & 3));   //this is a two bit variable
						 //DO NOT USE keysite_database[i].reserve_task_count = (0 | (d2 & 12)); //this is a two bit variable
						 keysite_database[i].requires_cap			= 	((d2 >> 6) & 0x01);
						 keysite_database[i].requires_barcap		= 	((d2 >> 7) & 0x01);
						 keysite_database[i].repairable				= 	((d2 >> 8) & 0x01);
						 keysite_database[i].oca_target				= 	((d2 >> 9) & 0x01);
						 keysite_database[i].recon_target			= 	((d2 >> 10) & 0x01);
						 keysite_database[i].ground_strike_target	= 	((d2 >> 11) & 0x01);
						 keysite_database[i].ship_strike_target	= 	((d2 >> 12) & 0x01);
						 keysite_database[i].troop_insertion_target =((d2 >> 13) & 0x01);
						 keysite_database[i].campaign_objective	= 	((d2 >> 14) & 0x01);       

// VJ 040122 WUT info sorry to make the code messy !                                         	                	
/* info from the help file of WUT
see file: \aphavoc\source\entity\special\keysite\ks_dbase.h
Binary (col.13)
The following data is packed into bit-fields
[Bits 1 to 6] Unknown  <== MAP STUFF 
[Bits 7 to 12] Unknown <== MAP STUFF 
[Bit 13] Unknown <== align with terrain 
[Bit 14] Unique Name, Specifies whether the keysite has a special name (e.g. Airbases all have individual names), or just a number (e.g. FARP #6)
[Bit 15] Hidden by Fog
[Bits 16 to 17] Unknown <== air_force_capacity = 2 bits
...
however these are 6 bits not 5!!!
 
[Bit 6] Requires CAP   < so barcap starts in fact at bit 7 not 6 etc!
[Bit 7] Requires BARCAP
If set then these tasks will be generated when necessary.
(Not sure if BARCAP actually works in the game)

[Bit 8] Repairable
Whether structures at the site are to be repaired or not
 
[Bit 9] OCA Target
[Bit 10] Recon Target
[Bit 11] Ground Strike Target
[Bit 12] Ship Strike Target
[Bit 13] Troop Insertion Target
Specifies whether the site can be a target of the listed mission types
