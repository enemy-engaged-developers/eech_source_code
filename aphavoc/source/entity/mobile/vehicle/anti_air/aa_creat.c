...
static entity *create_local (entity_types type, int index, char *pargs)
{
	...
	if (en)
	{
   	...
   	//
		...
		raw = (anti_aircraft *) malloc_fast_mem (sizeof (anti_aircraft));
   	...
   	// INITIALISE ALL ENTITY DATA TO 'WORKING' DEFAULT VALUES
		...
		memset (raw, 0, sizeof (anti_aircraft));
		...
		set_initial_rotation_angle_of_anti_aircraft_wheels (raw->vh.inst3d);
...
void overload_anti_aircraft_create_functions (void)
{
	fn_create_local_entity[ENTITY_TYPE_ANTI_AIRCRAFT]		  								= create_local;

	fn_create_client_server_entity[ENTITY_TYPE_ANTI_AIRCRAFT][COMMS_MODEL_SERVER]	= create_server;

	fn_create_client_server_entity[ENTITY_TYPE_ANTI_AIRCRAFT][COMMS_MODEL_CLIENT]	= create_client;
}
...
entity *create_client_server_entity_anti_aircraft (int index, entity_sub_types sub_type, entity *group, vec3d *position)
{
	entity
		*new_entity = nullptr;
...
entity *create_local_entity_anti_aircraft (int index, entity_sub_types sub_type, entity *group, vec3d *position)
{
	entity
		*new_entity = nullptr;
