...
static entity *create_local (entity_types type, int index, char *pargs)
{
	...
	if (en)
	{
   	...
   	// INITIALISE ALL ENTITY DATA TO 'WORKING' DEFAULT VALUES
		...
		raw->vh.target_scan_timer = vehicle_database[raw->vh.mob.sub_type].target_scan_delay * (1.0 + 0.5 * frand1());
...
void overload_person_create_functions (void)
{
	fn_create_local_entity[ENTITY_TYPE_PERSON]		  								= create_local;

	fn_create_client_server_entity[ENTITY_TYPE_PERSON][COMMS_MODEL_SERVER]	= create_server;

	fn_create_client_server_entity[ENTITY_TYPE_PERSON][COMMS_MODEL_CLIENT]	= create_client;
}
