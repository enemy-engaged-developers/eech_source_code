...
static entity *create_local (entity_types type, int index, char *pargs)
{
	...
	if (en)
	{
   	...
   	// INITIALISE ALL ENTITY DATA TO 'WORKING' DEFAULT VALUES
		...
		raw->vh.loading_door_state = VEHICLE_LOADING_DOORS_CLOSED_FLOAT_VALUE;
...
void overload_ship_vehicle_create_functions (void)
{
	fn_create_local_entity[ENTITY_TYPE_SHIP_VEHICLE]		  								= create_local;

	fn_create_client_server_entity[ENTITY_TYPE_SHIP_VEHICLE][COMMS_MODEL_SERVER]	= create_server;

	fn_create_client_server_entity[ENTITY_TYPE_SHIP_VEHICLE][COMMS_MODEL_CLIENT]	= create_client;
}
...
entity *create_client_server_entity_ship (int index, entity_sub_types sub_type, entity *group, vec3d *position)
{
	entity
		*new_entity = nullptr;
	...
	attach_ship_meta_smoke_lists (new_entity);
...
entity *create_local_entity_ship (int index, entity_sub_types sub_type, entity *group, vec3d *position)
{
	entity
		*new_entity = nullptr;
