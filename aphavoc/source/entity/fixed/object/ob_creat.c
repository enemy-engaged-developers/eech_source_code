...
static entity *create_local (entity_types type, int index, char *pargs)
{
	...
	if (en)
	{
   	...
   	//
		...
		raw = (object *) malloc_fast_mem (sizeof (object));
   	...
   	// INITIALISE ALL ENTITY DATA TO 'WORKING' DEFAULT VALUES
		...
		raw->object_scaling.x = 1.0;
		raw->object_scaling.y = 1.0;
		raw->object_scaling.z = 1.0;
...
void overload_object_create_functions (void)
{
	fn_create_local_entity[ENTITY_TYPE_OBJECT]		  								= create_local;

	fn_create_client_server_entity[ENTITY_TYPE_OBJECT][COMMS_MODEL_SERVER]	= create_server;

	fn_create_client_server_entity[ENTITY_TYPE_OBJECT][COMMS_MODEL_CLIENT]	= create_client;
}
