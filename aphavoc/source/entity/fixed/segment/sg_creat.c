...
static entity *create_local (entity_types type, int index, char *pargs)
{
	...
	debug_log ("SEGMENT: create %d", index);
	...
	if (en)
	{
   	...
   	//
		...
		raw = (segment *) malloc_fast_mem (sizeof (segment));
   	...
   	// INITIALISE ALL ENTITY DATA TO 'WORKING' DEFAULT VALUES
		...
		ASSERT (raw->segment_link.parent);
		...
		raw->altitude = raw->fix.position.y;		// used when repairing segment
		...
		insert_local_entity_into_parents_child_list (en, LIST_TYPE_SEGMENT, raw->segment_link.parent, raw->segment_link.child_pred);
...
void overload_segment_create_functions (void)
{
	fn_create_local_entity[ENTITY_TYPE_SEGMENT]		  								= create_local;

	fn_create_client_server_entity[ENTITY_TYPE_SEGMENT][COMMS_MODEL_SERVER]	= create_server;

	fn_create_client_server_entity[ENTITY_TYPE_SEGMENT][COMMS_MODEL_CLIENT]	= create_client;
}
