...
static entity *create_local (entity_types type, int index, char *pargs)
{
	...
	if (en)
	{
   	...
   	//
		...
		raw = (site_updatable *) malloc_fast_mem (sizeof (site_updatable));
...
void overload_site_updatable_create_functions (void)
{
	fn_create_local_entity[ENTITY_TYPE_SITE_UPDATABLE]		  									= create_local;

	fn_create_client_server_entity[ENTITY_TYPE_SITE_UPDATABLE][COMMS_MODEL_SERVER]	= create_server;

	fn_create_client_server_entity[ENTITY_TYPE_SITE_UPDATABLE][COMMS_MODEL_CLIENT]	= create_client;
}
