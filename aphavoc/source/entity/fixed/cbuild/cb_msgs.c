...
static int response_to_collision (entity_messages message, entity *receiver, entity *sender, va_list pargs)
{
	...
	{
		if ((sender) && (get_local_entity_type (sender) == ENTITY_TYPE_WEAPON))
		{
			aggressor = get_local_entity_parent (sender, LIST_TYPE_LAUNCHED_WEAPON);

			if (aggressor)
			{
				play_building_destroyed_speech (receiver, aggressor);
			}
		}
...
void overload_city_building_message_responses (void)
{
	overload_fixed_message_responses (ENTITY_TYPE_CITY_BUILDING);

	message_responses[ENTITY_TYPE_CITY_BUILDING][ENTITY_MESSAGE_COLLISION]		= response_to_collision;

	#if DEBUG_MODULE

	message_responses[ENTITY_TYPE_CITY_BUILDING][ENTITY_MESSAGE_LINK_CHILD]		= response_to_link_child;

	message_responses[ENTITY_TYPE_CITY_BUILDING][ENTITY_MESSAGE_UNLINK_CHILD]	= response_to_unlink_child;

	message_responses[ENTITY_TYPE_CITY_BUILDING][ENTITY_MESSAGE_LINK_PARENT]	= response_to_link_parent;

	message_responses[ENTITY_TYPE_CITY_BUILDING][ENTITY_MESSAGE_UNLINK_PARENT]	= response_to_unlink_parent;
