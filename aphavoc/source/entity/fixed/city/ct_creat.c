...
static entity *create_local (entity_types type, int index, char *pargs)
{
	...
	if (en)
	{
   	...
   	//
		...
		raw = (city *) malloc_fast_mem (sizeof (city));
   	...
   	// INITIALISE ALL ENTITY DATA TO 'WORKING' DEFAULT VALUES
		...
		raw->bounding_box_min.x = 1000000.0;
		raw->bounding_box_min.y = 1000000.0;
		raw->bounding_box_min.z = 1000000.0;

		raw->bounding_box_max.x = -1000000.0;
		raw->bounding_box_max.y = -1000000.0;
		raw->bounding_box_max.z = -1000000.0;
...
void overload_city_create_functions (void)
{
	fn_create_local_entity[ENTITY_TYPE_CITY]		  								= create_local;

	fn_create_client_server_entity[ENTITY_TYPE_CITY][COMMS_MODEL_SERVER]	= create_server;

	fn_create_client_server_entity[ENTITY_TYPE_CITY][COMMS_MODEL_CLIENT]	= create_client;
}
