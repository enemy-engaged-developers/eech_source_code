...
static entity *create_local (entity_types type, int index, char *pargs)
{
	...
	if (en)
	{
   	...
   	//
		...
		raw = (site *) malloc_fast_mem (sizeof (site));
   	...
   	// INITIALISE ALL ENTITY DATA TO 'WORKING' DEFAULT VALUES
		...
		if (!fixed_3d_object_database[raw->fix.object_3d_shape].valid_entry)
		{
			...
			debug_filtered_log ("ST_CREAT: Warning no entry for %s (%d)", object_3d_enumeration_names [raw->fix.object_3d_shape], raw->fix.object_3d_shape);
		}
		else
		{
...
void overload_site_create_functions (void)
{
	fn_create_local_entity[ENTITY_TYPE_SITE]		  								= create_local;

	fn_create_client_server_entity[ENTITY_TYPE_SITE][COMMS_MODEL_SERVER]	= create_server;

	fn_create_client_server_entity[ENTITY_TYPE_SITE][COMMS_MODEL_CLIENT]	= create_client;
}
