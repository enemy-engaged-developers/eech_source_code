...
static entity *create_local (entity_types type, int index, char *pargs)
{
	...
	if (en)
	{
   	...
   	//
		...
		raw = (bridge *) malloc_fast_mem (sizeof (bridge));
...
void overload_bridge_create_functions (void)
{
	fn_create_local_entity[ENTITY_TYPE_BRIDGE]		  								= create_local;

	fn_create_client_server_entity[ENTITY_TYPE_BRIDGE][COMMS_MODEL_SERVER]	= create_server;

	fn_create_client_server_entity[ENTITY_TYPE_BRIDGE][COMMS_MODEL_CLIENT]	= create_client;
}
