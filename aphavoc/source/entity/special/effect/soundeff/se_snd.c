...
//			v = maximum_volume * ((temp * temp) / (zero_volume_range * zero_volume_range));
	...
	if (get_local_entity_int_value (en, INT_TYPE_ENTITY_SUB_TYPE) == ENTITY_SUB_TYPE_EFFECT_SOUND_RADIO_MESSAGE && command_line_radio_msgs_noise)
	{
		float radio_noize = pow(raw->amplification, 1.0f - (float)(frand1()/2));
		v *= radio_noize;
	}
	else
		v *= raw->amplification;
	...
	if (in_cockpit)
	{
		...
		case ENTITY_SUB_TYPE_EFFECT_SOUND_RADIO_MESSAGE:
		case ENTITY_SUB_TYPE_EFFECT_SOUND_WARNING_MESSAGE:
		case ENTITY_SUB_TYPE_EFFECT_SOUND_MUSIC:
			channel = 1;
			// FALLTHROUGH
