...
void reset_end_of_mission_camera (camera *raw)
{
	ASSERT (raw);

	ASSERT (raw->external_view_entity);

	raw->end_of_mission_camera_heading = 0.0;
...
void update_end_of_mission_camera (camera *raw)
{
	...
	{
		terrain_elevation = get_3d_terrain_point_data (raw->position.x, raw->position.z, &raw->terrain_info);

		terrain_class = get_terrain_type_class (get_3d_terrain_point_data_type (&raw->terrain_info));

		if
		(
			(terrain_class == TERRAIN_CLASS_WATER) &&
			(!get_local_entity_int_value (en, INT_TYPE_ALIVE)) &&
			(get_local_entity_float_value (en, FLOAT_TYPE_RADAR_ALTITUDE) < 1.0)
		)
		{
			raw->position.y = terrain_elevation;

			rel_camera_position.x = 0.0;
			rel_camera_position.y = 0.0;
			rel_camera_position.z = -100.0;
