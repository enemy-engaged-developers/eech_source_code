...
//VJ 050131 update on wideview mod, much better movement
		...
		if (get_local_entity_int_value (get_pilot_entity (), INT_TYPE_CREW_ROLE) == CREW_ROLE_PILOT)
			mfd_light_power = (float)(get_comanche_main_mfd_mode(COMANCHE_MAIN_MFD_LOCATION_PILOT_LHS) % 2 + !!get_comanche_main_mfd_mode(COMANCHE_MAIN_MFD_LOCATION_PILOT_LHS) +
					get_comanche_main_mfd_mode(COMANCHE_MAIN_MFD_LOCATION_PILOT_RHS) % 2 + !!get_comanche_main_mfd_mode(COMANCHE_MAIN_MFD_LOCATION_PILOT_RHS)) / 4.0;
		else if (get_local_entity_int_value (get_pilot_entity (), INT_TYPE_CREW_ROLE) == CREW_ROLE_CO_PILOT)
			mfd_light_power = (float)(get_comanche_main_mfd_mode(COMANCHE_MAIN_MFD_LOCATION_CO_PILOT_LHS) % 2 + !!get_comanche_main_mfd_mode(COMANCHE_MAIN_MFD_LOCATION_CO_PILOT_LHS) +
					get_comanche_main_mfd_mode(COMANCHE_MAIN_MFD_LOCATION_CO_PILOT_RHS) % 2 + !!get_comanche_main_mfd_mode(COMANCHE_MAIN_MFD_LOCATION_CO_PILOT_RHS)) / 4.0;

		ASSERT(mfd_light_power >= 0.0 && mfd_light_power <= 1.0);

		if (mfd_light_power)
		{
			direction = get_cockpit_backlight_direction(virtual_cockpit_inst3d, mfd_light_power);
