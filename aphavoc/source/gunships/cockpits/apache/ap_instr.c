...
#define ALTITUDE_COUNTER_WIDTH 32
#define ALTITUDE_COUNTER_HEIGHT 16
...
void initialise_apache_instrument_colours (void)
{
...
void set_apache_instrument_colours (void)
{
...
void draw_apache_fixed_cockpit_airspeed_indicator_needle (cockpit_panels panel)
{
	float
		airspeed_needle_value,
		x_centre,
...
void draw_apache_fixed_cockpit_barometric_altimeter_needle (cockpit_panels panel)
{
	float
		altimeter_needle_value,
		x_centre,
...
void draw_apache_fixed_cockpit_clock_hands (cockpit_panels panel)
{
	float
		hour_hand_value,
		minute_hand_value,
...
void get_apache_virtual_cockpit_adi_angles (matrix3x3 attitude, float *heading, float *pitch, float *roll)
{
	matrix3x3
		inverse_attitude,
		heading_rotation,
...
float get_apache_virtual_cockpit_adi_slip_indicator_needle_value (void)
{
	float
		slip_needle_value;
...
float get_apache_virtual_cockpit_airspeed_indicator_needle_value (void)
{
	float
		airspeed_needle_value,
		fraction,
...
float get_apache_virtual_cockpit_barometric_altimeter_needle_value (void)
{
	float
		altimeter_needle_value;
...
void get_apache_virtual_cockpit_clock_hand_values (float *hours, float *minutes, float *seconds)
{
	float
		hour_hand_value,
		minute_hand_value,
...
static void draw_altitude_counter_digits(void)
{
	...
	x_min = x_org;
	y_min = y_org;
	x_max = x_org + ALTITUDE_COUNTER_WIDTH;
	y_max = y_org + ALTITUDE_COUNTER_HEIGHT;
...
void initialise_altitude_counter(void)
{
	altitude_counter_screen = create_system_texture_screen(ALTITUDE_COUNTER_WIDTH, ALTITUDE_COUNTER_HEIGHT, TEXTURE_INDEX_AVCKPT_ALTITUDE_COUNTER, TEXTURE_TYPE_NOALPHA);
}
