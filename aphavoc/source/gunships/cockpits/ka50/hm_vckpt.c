...
void draw_ka50_internal_virtual_cockpit (unsigned int flags)
{
	viewpoint
		vp;
...
//VJ wideview mod, date: 18-mar-03	
	...
	{
		draw_ka50_hud_on_texture ();
	}

	if (flags & VIRTUAL_COCKPIT_EKRAN_DISPLAY)
	{
		draw_ka50_ekran_display_on_texture ();
	}
...
void draw_ka50_external_virtual_cockpit (unsigned int flags, unsigned char *wiper_rle_graphic)
{
	viewpoint
		vp;
	...
	if (flags & (VIRTUAL_COCKPIT_MAIN_ROTOR | VIRTUAL_COCKPIT_STOWED_WIPER | VIRTUAL_COCKPIT_MOVING_WIPER))
	{
		...
		if (begin_3d_scene ())
		{
			...
			if (wiper_mode == WIPER_MODE_STOWED)
			{
				...
				{
					draw_ka50_virtual_cockpit_wiper (&vp, flags & VIRTUAL_COCKPIT_LARGE_HUD);
				}
			}
			else
			{
		...
		if (begin_3d_scene ())
		{
			...
			if (flags & VIRTUAL_COCKPIT_ADI)
			{
				...
				if (find_object_3d_sub_object (&search) == SUB_OBJECT_SEARCH_RESULT_OBJECT_FOUND)
				{
					...
					get_ka50_virtual_cockpit_adi_angles (vp.attitude, &heading, &pitch, &roll);
...
//VJ wideview mod, date: 18-mar-03	
	...
	if (flags & VIRTUAL_COCKPIT_RAIN_EFFECT)
	{
		if (rain_mode != RAIN_MODE_DRY)
		{
			...
			if (begin_3d_scene ())
			{
				draw_ka50_virtual_cockpit_rain_effect (&vp, flags & VIRTUAL_COCKPIT_LARGE_HUD);
...
void damage_ka50_virtual_cockpit_main_rotors (int seed)
{
	float
		theta;
...
void restore_ka50_virtual_cockpit_main_rotors (void)
{
	float
		theta;
