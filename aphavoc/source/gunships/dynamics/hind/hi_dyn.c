...
void initialise_hind_advanced_dynamics (entity *en)
{
	...
	current_flight_dynamics->sub_type = ENTITY_SUB_TYPE_AIRCRAFT_MI24D_HIND;

	sprintf (current_flight_dynamics->filename, "..\\common\\data\\hind.dyn");
}
...
/*
	{
		...
		if (keysite)
		{
			...
			if (get_local_entity_int_value (keysite, INT_TYPE_ENTITY_SUB_TYPE) == ENTITY_SUB_TYPE_KEYSITE_ANCHORAGE)
			{
				...
				debug_log ("HA_DYN: raising Havoc because landed on carrier");
...
void update_hind_advanced_dynamics (void)
{
