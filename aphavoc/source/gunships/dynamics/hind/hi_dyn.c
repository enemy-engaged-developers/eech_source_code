	...
	else
	{
		...
		while (option < NUM_DYNAMICS_OPTIONS_TYPES)
		{
			...
			if (flag & (1 << option))
			{
				...
				set_current_dynamics_options ((dynamics_options_types) option, TRUE);
...
//	current_flight_dynamics->input_data.cyclic_y_trim.value = 6.0;
	...
	current_flight_dynamics->main_rotor_diameter.value = 17.3f;
	...
	current_flight_dynamics->tail_rotor_diameter.value = 3.9f;
	...
	current_flight_dynamics->fuel_weight.max = 1720.0;
	current_flight_dynamics->fuel_weight.delta = 0.1720; // kg/s
	...
	{
		centre_of_gravity.y = -0.05f;
		centre_of_gravity.z = -0.005f;
	}
	else
	{
		centre_of_gravity.y = -0.2f;
...
void update_main_rotor_dynamics (void)
{
	...
	coll = current_flight_dynamics->input_data.collective.value / 120.0;
	coll *= coll;

	average_blade_input = bound (coll, 0.0, 1.0);
