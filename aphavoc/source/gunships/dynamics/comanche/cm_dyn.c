...
void initialise_comanche_advanced_dynamics (entity *en)
{
	...
	sprintf (current_flight_dynamics->filename, "..\\common\\data\\comanche.dyn");
}
...
void update_comanche_advanced_dynamics (void)
{
...
//			start_game_exit (GAME_EXIT_CRASH, FALSE);
		...
		(!(current_flight_dynamics->dynamics_damage & DYNAMICS_DAMAGE_MAIN_ROTOR)))
	{
		...
		{
			debug_log("Vortex effect: %f", air_over_rotor);
...
void comanche_restore_damage_values (void)
{

	current_flight_dynamics->centre_of_gravity.x = 0.00;
	current_flight_dynamics->centre_of_gravity.y = -0.2; 
	current_flight_dynamics->centre_of_gravity.z = -0.008;
}
