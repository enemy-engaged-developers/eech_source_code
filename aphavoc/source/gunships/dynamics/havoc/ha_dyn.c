...
void set_dynamics_defaults (entity *en)
{
	...
	current_flight_dynamics->main_rotor_induced_air.modifier = 0.6f;
...
*/
	...
	current_flight_dynamics->mass.min = 7900.0;
	current_flight_dynamics->mass.max = 0.0;

	current_flight_dynamics->fuel_weight.value = 1337.0;
	current_flight_dynamics->fuel_weight.min = 0.0;
	current_flight_dynamics->fuel_weight.max = 1337.0;
...
//			start_game_exit (GAME_EXIT_CRASH, FALSE);
	...
	if (command_line_dynamics_flight_model != 2 && !get_local_entity_int_value (get_gunship_entity (), INT_TYPE_AIRBORNE_AIRCRAFT))
	{
		...
		rotor_force = main_rotor_induced_air_value * 10.0;
