...
void initialise_hokum_advanced_dynamics (entity *en)
{
	...
	current_flight_dynamics->sub_type = ENTITY_SUB_TYPE_AIRCRAFT_KA52_HOKUM_B;

	sprintf (current_flight_dynamics->filename, "..\\common\\data\\hokum.dyn");
}
...
void set_dynamics_defaults (entity *en)
{
	...
	current_flight_dynamics->translational_lift.value = 3;
	...
	current_flight_dynamics->landing_quality.value = -7.0;
	current_flight_dynamics->landing_quality.max = -10.0;
...
void update_hokum_advanced_dynamics (void)
{
...
void update_left_engine_rpm_dynamics (void)
{
	...
	if (get_current_dynamics_options (DYNAMICS_OPTIONS_OVER_TORQUE))
	{
		...
		if (current_flight_dynamics->left_engine_torque.value > 101.0)
		{
			...
			current_flight_dynamics->left_engine_temp.value += 0.3 * (current_flight_dynamics->left_engine_torque.value - 101.0) * get_model_delta_time ();
...
void update_right_engine_rpm_dynamics (void)
{
	...
	if (get_current_dynamics_options (DYNAMICS_OPTIONS_OVER_TORQUE))
	{
		...
		if (current_flight_dynamics->right_engine_torque.value > 101.0)
		{
			...
			current_flight_dynamics->right_engine_temp.value += 0.37 * (current_flight_dynamics->right_engine_torque.value - 101.0) * get_model_delta_time ();
...
//			start_game_exit (GAME_EXIT_CRASH, FALSE);
	...
	if (!get_local_entity_int_value (get_gunship_entity (), INT_TYPE_AIRBORNE_AIRCRAFT))
	{
		...
		rotor_force = main_rotor_induced_air_value * 35.0;
...
/*
		...
		{
			float 
				horizontal_motion_vector_magnitude,
				motion_vector_pitch;
			...
			horizontal_motion_vector_magnitude = (get_3d_vector_magnitude (&temp_model_motion_vector));
			...
			force = horizontal_motion_vector_magnitude *
						((pitch_value +
						main_rotor_pitch_angle_value) -
						motion_vector_pitch);
	...
	////////////////////////////////////////////
	// main rotor disc torque effect 1
	////////////////////////////////////////////
	//if (!model_landed)
	{
	...
	////////////////////////////////////////////
	// main rotor disc torque effect 2
	////////////////////////////////////////////
	//if (!model_landed)
	{

		if (!current_flight_dynamics->main_rotor_rpm.damaged)
		{
	...
	//if (!model_landed)
	{
		...
		//reaction_force += (0.0 - main_angular_force) * scaling;
		...
		(!(current_flight_dynamics->dynamics_damage & DYNAMICS_DAMAGE_MAIN_ROTOR)))
	{
		...
		reaction_force = 90.0 * (current_flight_dynamics->pitch.delta + current_flight_dynamics->main_rotor_pitch_angle.delta);
	...
	////////////////////////////////////////////
	{
		...
		if (current_flight_dynamics->main_rotor_rpm.max != 0.0)
		{
			...
			Fmax = drag + 4.5 * (main_rotor_rpm_value / current_flight_dynamics->main_rotor_rpm.max);
		}
		...
		if (keysite)
		{
			...
			if (get_local_entity_int_value (keysite, INT_TYPE_ENTITY_SUB_TYPE) == ENTITY_SUB_TYPE_KEYSITE_ANCHORAGE)
			{
				...
				if (helicopter_within_keysite_area (get_gunship_entity ()))
				{
					...
					if (current_flight_dynamics->position.y < 16.6136 + get_local_entity_float_value (get_gunship_entity (), FLOAT_TYPE_CENTRE_OF_GRAVITY_TO_GROUND_DISTANCE))
					{
						...
						current_flight_dynamics->position.y = bound (current_flight_dynamics->position.y, 16.6136 + get_local_entity_float_value (get_gunship_entity (), FLOAT_TYPE_CENTRE_OF_GRAVITY_TO_GROUND_DISTANCE), MAX_MAP_Y);
					}
				}
			}
		}
...
void hokum_restore_damage_values (void)
{
