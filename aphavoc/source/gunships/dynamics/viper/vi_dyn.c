...
void initialise_viper_advanced_dynamics (entity *en)
{
	ASSERT (en);
	...
	current_flight_dynamics->sub_type = ENTITY_SUB_TYPE_AIRCRAFT_AH1Z_VIPER;

	if (command_line_dynamics_flight_model != 2)
		sprintf (current_flight_dynamics->filename, "..\\common\\data\\viper.dyn");
	else
		sprintf (current_flight_dynamics->filename, "..\\common\\data\\dynamics\\viper.dyn");
}
...
void update_viper_advanced_dynamics (void)
{
...
void viper_restore_damage_values (void)
{
