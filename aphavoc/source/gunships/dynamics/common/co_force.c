   ...
   ///////////////////////////////////////////////////////////////
	{
        ...
        else
        {
				...
				if (command_line_dynamics_flight_model >= 2)
				{
					...
					current_flight_dynamics->input_data.cyclic_y.value = bound((current_flight_dynamics->pitch.value - rad(5.0f))/ rad(10.0) * 100.0, current_flight_dynamics->input_data.cyclic_y.min, current_flight_dynamics->input_data.cyclic_y.max);
				}

				current_flight_dynamics->model_acceleration_vector.x = -(10.0f * get_model_delta_time ()) * current_flight_dynamics->model_motion_vector.x;
				current_flight_dynamics->model_acceleration_vector.y = resultant_vertically;
				current_flight_dynamics->model_acceleration_vector.z = -(10.0f * get_model_delta_time ()) * current_flight_dynamics->model_motion_vector.z;
...
void update_collision_dynamics (void)
{
	...
	if (fixed_collision_count == 0)
	{
		if (!get_local_entity_int_value (get_gunship_entity (), INT_TYPE_AIRBORNE_AIRCRAFT))
		{
			...
			if (current_flight_dynamics->world_motion_vector.y > 0.0  && !weight_on_wheels())
			{
