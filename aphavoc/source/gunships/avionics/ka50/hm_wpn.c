...
void update_ka50_weapon_systems (void)
{
	int
		inhibit_launch;
...
void toggle_ka50_radar_jammer_manual (void)
{
	...
	play_common_cpg_radar_jammer_speech (ka50_damage.radar_jammer);
}
...
void set_ka50_radar_jammer_auto (int state)
{
	if (!ka50_damage.radar_jammer)
	{
		if (get_global_auto_counter_measures ())
...
void set_ka50_infra_red_jammer_manual (int state)
{
	if (!ka50_damage.infra_red_jammer)
	{
		if (!get_global_auto_counter_measures ())
