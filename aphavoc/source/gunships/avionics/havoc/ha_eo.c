...
void initialise_havoc_eo (void)
{
	...
	eo_azimuth							= rad (0.0);
	eo_min_azimuth						= rad (-110.0f);
	eo_max_azimuth						= rad (110.0f);
	eo_elevation						= rad (0.0);
	eo_min_elevation					= rad (-40.0f);
...
void inc_havoc_eo_zoom(void)
{
	switch (eo_sensor)
	{
		case TARGET_ACQUISITION_SYSTEM_FLIR:
			dec_eo_field_of_view(&havoc_flir);
			break;
		case TARGET_ACQUISITION_SYSTEM_LLLTV:
			dec_eo_field_of_view(&havoc_llltv);
			break;
		default:
			break;
	}
...
void dec_havoc_eo_zoom(void)
{
	switch (eo_sensor)
	{
		case TARGET_ACQUISITION_SYSTEM_FLIR:
			inc_eo_field_of_view(&havoc_flir);
			break;
		case TARGET_ACQUISITION_SYSTEM_LLLTV:
			inc_eo_field_of_view(&havoc_llltv);
			break;
		default:
			break;
	}
...
void toggle_havoc_eo_system(void)
{
	switch (eo_sensor)
	{
	case TARGET_ACQUISITION_SYSTEM_FLIR:
