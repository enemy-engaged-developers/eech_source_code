...
static void select_target_acquisition_system_ground_radar_event (event *ev)
{
	...
	{
		select_havoc_target_acquisition_system (TARGET_ACQUISITION_SYSTEM_GROUND_RADAR);
	}
}
...
static void select_target_acquisition_system_flir_event (event *ev)
{
	if (!get_global_simple_avionics ())
	{
		select_havoc_target_acquisition_system (TARGET_ACQUISITION_SYSTEM_FLIR);
	}
}
...
static void select_target_acquisition_system_llltv_event (event *ev)
{
	if (!get_global_simple_avionics ())
	{
		select_havoc_target_acquisition_system (TARGET_ACQUISITION_SYSTEM_LLLTV);
	}
}
...
{
	centre_havoc_target_acquisition_system ();
}
...
{
	toggle_havoc_show_allied_targets ();
}
...
{
	havoc_target_acquisition_system_misc_function1 ();
}
...
{
	havoc_target_acquisition_system_misc_function2 ();
}
...
{
	deactivate_havoc_master_caution_lamp ();
}
...
{
	toggle_havoc_infra_red_jammer_manual ();
}
