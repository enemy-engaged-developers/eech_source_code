...
#define HIND_MOVING_MAP_RANGE_50000	((float) 50000.01)
		...
		if (doaspect)
	  	{
			...
			aspectrow = (float*)safe_malloc(sizeof(float)*nr_aspect_steps);
			old_aspectrow = (float*)safe_malloc(sizeof(float)*nr_aspect_steps);
		}
...
void draw_tsd_contour_map (env_2d *mfd_env, float y_translate, float range, float scale, vec3d *position, float heading, int draw_large_mfd, int force_contours)
{
	...
	////////////////////////////////////////
	else if (range == TSD_ASE_RANGE_50000)
	////////////////////////////////////////
	{
		if (draw_large_mfd)
		{
