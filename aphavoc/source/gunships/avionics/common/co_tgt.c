...
char *get_target_display_name (entity *target, char *buffer, int large_display)
{
	int
		allied;
...
void play_common_cpg_target_acquisition_system_speech (target_acquisition_systems new_system, target_acquisition_systems old_system, int damaged)
{
	switch (new_system)
	{
		...
		////////////////////////////////////////
		{
			...
			{
				if ((old_system != TARGET_ACQUISITION_SYSTEM_GROUND_RADAR) && (old_system != TARGET_ACQUISITION_SYSTEM_AIR_RADAR))
				{
