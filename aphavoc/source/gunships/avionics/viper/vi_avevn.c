...
{
	centre_viper_target_acquisition_system ();
}
...
{
	toggle_viper_show_allied_targets ();
}
...
{
	deactivate_viper_master_caution ();
}
...
static void select_next_tsd_ase_range_event (event *ev)
{
	...
	{
		select_next_viper_pilot_tsd_ase_range ();
	}
	else
	{
		select_next_viper_co_pilot_tsd_ase_range ();
	}
}
...
static void select_previous_tsd_ase_range_event (event *ev)
{
	if (get_crew_role () == CREW_ROLE_PILOT)
	{
		select_previous_viper_pilot_tsd_ase_range ();
	}
	else
	{
		select_previous_viper_co_pilot_tsd_ase_range ();
	}
}
...
static void select_next_tsd_declutter_level_event (event *ev)
{
	if (get_crew_role () == CREW_ROLE_PILOT)
	{
		select_next_viper_pilot_tsd_declutter_level ();
	}
	else
	{
		select_next_viper_co_pilot_tsd_declutter_level ();
	}
}
...
static void select_previous_tsd_declutter_level_event (event *ev)
{
	if (get_crew_role () == CREW_ROLE_PILOT)
	{
		select_previous_viper_pilot_tsd_declutter_level ();
	}
	else
	{
		select_previous_viper_co_pilot_tsd_declutter_level ();
	}
}
...
static void select_next_lhs_mfd_event (event *ev)
{
	if (get_crew_role () == CREW_ROLE_PILOT)
	{
		select_next_viper_mfd (VIPER_MFD_LOCATION_PILOT_LHS);
	}
	else
	{
		select_next_viper_mfd (VIPER_MFD_LOCATION_CO_PILOT_LHS);
	}
}
...
static void select_previous_lhs_mfd_event (event *ev)
{
	if (get_crew_role () == CREW_ROLE_PILOT)
	{
		select_previous_viper_mfd (VIPER_MFD_LOCATION_PILOT_LHS);
	}
	else
	{
		select_previous_viper_mfd (VIPER_MFD_LOCATION_CO_PILOT_LHS);
	}
}
...
static void select_next_rhs_mfd_event (event *ev)
{
	if (get_crew_role () == CREW_ROLE_PILOT)
	{
		select_next_viper_mfd (VIPER_MFD_LOCATION_PILOT_RHS);
	}
	else
	{
		select_next_viper_mfd (VIPER_MFD_LOCATION_CO_PILOT_RHS);
	}
}
...
static void select_previous_rhs_mfd_event (event *ev)
{
	if (get_crew_role () == CREW_ROLE_PILOT)
	{
		select_previous_viper_mfd (VIPER_MFD_LOCATION_PILOT_RHS);
	}
	else
	{
		select_previous_viper_mfd (VIPER_MFD_LOCATION_CO_PILOT_RHS);
	}
}
...
static void select_next_dfd_event (event *ev)
{
	if (get_crew_role () == CREW_ROLE_PILOT)
	{
		select_next_viper_mfd (VIPER_MFD_LOCATION_PILOT_DFD);
	}
	else
	{
		select_next_viper_mfd (VIPER_MFD_LOCATION_CO_PILOT_DFD);
	}
}
...
static void toggle_dfd_on_off_event (event *ev)
{
	if (get_crew_role () == CREW_ROLE_PILOT)
	{
		toggle_viper_mfd_on_off (VIPER_MFD_LOCATION_PILOT_DFD);
	}
	else
	{
		toggle_viper_mfd_on_off (VIPER_MFD_LOCATION_CO_PILOT_DFD);
	}
}
...
static void select_lhs_tads_mfd_event (event *ev)
{
	if (eo_sensor == TARGET_ACQUISITION_SYSTEM_FLIR)
	{
		if (get_crew_role () == CREW_ROLE_PILOT)
		{
			select_viper_mfd_mode (VIPER_MFD_MODE_FLIR, VIPER_MFD_LOCATION_PILOT_LHS);
		}
		else
		{
			select_viper_mfd_mode (VIPER_MFD_MODE_FLIR, VIPER_MFD_LOCATION_CO_PILOT_LHS);
		}
	}
	else
	{
		if (get_crew_role () == CREW_ROLE_PILOT)
		{
			select_viper_mfd_mode (VIPER_MFD_MODE_DTV, VIPER_MFD_LOCATION_PILOT_LHS);
		}
		else
		{
			select_viper_mfd_mode (VIPER_MFD_MODE_DTV, VIPER_MFD_LOCATION_CO_PILOT_LHS);
		}
	}
}
...
static void select_rhs_tads_mfd_event (event *ev)
{
	if (eo_sensor == TARGET_ACQUISITION_SYSTEM_FLIR)
	{
		if (get_crew_role () == CREW_ROLE_PILOT)
		{
			select_viper_mfd_mode (VIPER_MFD_MODE_FLIR, VIPER_MFD_LOCATION_PILOT_RHS);
		}
		else
		{
			select_viper_mfd_mode (VIPER_MFD_MODE_FLIR, VIPER_MFD_LOCATION_CO_PILOT_RHS);
		}
	}
	else
	{
		if (get_crew_role () == CREW_ROLE_PILOT)
		{
			select_viper_mfd_mode (VIPER_MFD_MODE_DTV, VIPER_MFD_LOCATION_PILOT_RHS);
		}
		else
		{
			select_viper_mfd_mode (VIPER_MFD_MODE_DTV, VIPER_MFD_LOCATION_CO_PILOT_RHS);
		}
	}
}
...
static void select_lhs_tsd_mfd_event (event *ev)
{
	if (get_crew_role () == CREW_ROLE_PILOT)
	{
		select_viper_mfd_mode (VIPER_MFD_MODE_TSD, VIPER_MFD_LOCATION_PILOT_LHS);
	}
	else
	{
		select_viper_mfd_mode (VIPER_MFD_MODE_TSD, VIPER_MFD_LOCATION_CO_PILOT_LHS);
	}
}
...
static void select_lhs_weapon_mfd_event (event *ev)
{
	if (get_crew_role () == CREW_ROLE_PILOT)
	{
		select_viper_mfd_mode (VIPER_MFD_MODE_WEAPON, VIPER_MFD_LOCATION_PILOT_LHS);
	}
	else
	{
		select_viper_mfd_mode (VIPER_MFD_MODE_WEAPON, VIPER_MFD_LOCATION_CO_PILOT_LHS);
	}
}
...
static void select_rhs_weapon_mfd_event (event *ev)
{
	if (get_crew_role () == CREW_ROLE_PILOT)
	{
		select_viper_mfd_mode (VIPER_MFD_MODE_WEAPON, VIPER_MFD_LOCATION_PILOT_RHS);
	}
	else
	{
		select_viper_mfd_mode (VIPER_MFD_MODE_WEAPON, VIPER_MFD_LOCATION_CO_PILOT_RHS);
	}
}
...
static void select_lhs_system_mfd_event (event *ev)
{
	if (get_crew_role () == CREW_ROLE_PILOT)
	{
		select_viper_mfd_mode (VIPER_MFD_MODE_SYSTEM, VIPER_MFD_LOCATION_PILOT_LHS);
	}
	else
	{
		select_viper_mfd_mode (VIPER_MFD_MODE_SYSTEM, VIPER_MFD_LOCATION_CO_PILOT_LHS);
	}
}
...
static void select_rhs_system_mfd_event (event *ev)
{
	if (get_crew_role () == CREW_ROLE_PILOT)
	{
		select_viper_mfd_mode (VIPER_MFD_MODE_SYSTEM, VIPER_MFD_LOCATION_PILOT_RHS);
	}
	else
	{
		select_viper_mfd_mode (VIPER_MFD_MODE_SYSTEM, VIPER_MFD_LOCATION_CO_PILOT_RHS);
	}
}
...
static void select_lhs_engine_mfd_event (event *ev)
{
	if (get_crew_role () == CREW_ROLE_PILOT)
	{
		select_viper_mfd_mode (VIPER_MFD_MODE_ENGINE, VIPER_MFD_LOCATION_PILOT_LHS);
	}
	else
	{
		select_viper_mfd_mode (VIPER_MFD_MODE_ENGINE, VIPER_MFD_LOCATION_CO_PILOT_LHS);
	}
}
...
static void select_rhs_engine_mfd_event (event *ev)
{
	if (get_crew_role () == CREW_ROLE_PILOT)
	{
		select_viper_mfd_mode (VIPER_MFD_MODE_ENGINE, VIPER_MFD_LOCATION_PILOT_RHS);
	}
	else
	{
		select_viper_mfd_mode (VIPER_MFD_MODE_ENGINE, VIPER_MFD_LOCATION_CO_PILOT_RHS);
	}
}
...
static void select_lhs_flight_mfd_event (event *ev)
{
	if (get_crew_role () == CREW_ROLE_PILOT)
	{
		select_viper_mfd_mode (VIPER_MFD_MODE_FLIGHT, VIPER_MFD_LOCATION_PILOT_LHS);
	}
	else
	{
		select_viper_mfd_mode (VIPER_MFD_MODE_FLIGHT, VIPER_MFD_LOCATION_CO_PILOT_LHS);
	}
}
...
static void select_rhs_flight_mfd_event (event *ev)
{
	if (get_crew_role () == CREW_ROLE_PILOT)
	{
		select_viper_mfd_mode (VIPER_MFD_MODE_FLIGHT, VIPER_MFD_LOCATION_PILOT_RHS);
	}
	else
	{
		select_viper_mfd_mode (VIPER_MFD_MODE_FLIGHT, VIPER_MFD_LOCATION_CO_PILOT_RHS);
	}
}
...
static void select_lhs_mission_mfd_event (event *ev)
{
	if (get_crew_role () == CREW_ROLE_PILOT)
	{
		select_viper_mfd_mode (VIPER_MFD_MODE_MISSION, VIPER_MFD_LOCATION_PILOT_LHS);
	}
	else
	{
...
static void select_rhs_mission_mfd_event (event *ev)
{
	if (get_crew_role () == CREW_ROLE_PILOT)
	{
		select_viper_mfd_mode (VIPER_MFD_MODE_MISSION, VIPER_MFD_LOCATION_PILOT_RHS);
	}
	else
	{
		select_viper_mfd_mode (VIPER_MFD_MODE_MISSION, VIPER_MFD_LOCATION_CO_PILOT_RHS);
	}
}
...
static void toggle_lhs_mfd_on_off_event (event *ev)
{
	if (get_crew_role () == CREW_ROLE_PILOT)
	{
		toggle_viper_mfd_on_off (VIPER_MFD_LOCATION_PILOT_LHS);
	}
	else
	{
		toggle_viper_mfd_on_off (VIPER_MFD_LOCATION_CO_PILOT_LHS);
	}
}
...
static void toggle_rhs_mfd_on_off_event (event *ev)
{
	if (get_crew_role () == CREW_ROLE_PILOT)
	{
		toggle_viper_mfd_on_off (VIPER_MFD_LOCATION_PILOT_RHS);
	}
	else
	{
		toggle_viper_mfd_on_off (VIPER_MFD_LOCATION_CO_PILOT_RHS);
	}
}
...
{
	toggle_viper_radar_jammer_manual ();
}
...
{
	toggle_viper_infra_red_jammer_manual ();
}
...
{
	if (!laser_is_active() && !viper_damage.laser_designator)
...
void set_viper_avionics_events (void)
{
...
//	set_event (DIK_DIVIDE, MODIFIER_NONE, KEY_STATE_DOWN, target_acquisition_system_misc_function2_event);
	...
	set_event (DIK_LBRACKET, MODIFIER_LEFT_ALT, KEY_STATE_DOWN, toggle_dfd_on_off_event);
	set_event (DIK_RBRACKET, MODIFIER_LEFT_ALT, KEY_STATE_DOWN, select_next_dfd_event);
	...
	set_event (DIK_C, MODIFIER_LEFT_ALT, KEY_STATE_DOWN, toggle_viper_canopy_doors);
